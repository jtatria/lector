...
/**
 * An analysis component that writes CAS data to files on disk in their serialized binary form.
 * 
 * Supports all formats listed in @link{BinaryFormat}.
 * 
 * Inspired by DKPro's binary format collection writers.
 *
    ...
    public void realProcess( JCas jcas ) throws AnalysisEngineProcessException {
        ...
        getLogger().log( Level.INFO, "Writing {0} CAS to {1}{2}{3}"
            , new Object[]{ delta ? "delta" : "full", 
                this.outputDir, File.separator, getFileNameForCas( jcas.getCas() ) 
            }
        );
