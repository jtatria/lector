...
/**
 * Default text filter buffer implementation.
 * 
 * This class implements a buffer filter capable of carrying out two major operations:
 * String substitutions, according to a set of regular expression patterns and new-line
 * normalization following XML mark up instead of in-text new-line markup.
 * 
...
public class InciteTextFilter extends Resource_ImplBase implements TextFilter {
    ...
    /**
     * Regexp-string pairs for regular expression formatting 
     * 
     * The string corresponding to the character sequence read in via the 
     * {@link #consume(char[], int, int) } method will be tested against each of the regular 
     * expressions in the odd positions in the given array and replace matches with the string 
     * value found at the corresponding even position in the given string array.
     * 
     * Default value will normalize all contigous whitespace to be equal to one space character.
     **/
...
//        "(\\p{Alnum})\\s+(\\p{Punct})", "$1$2", // remove space before punctuation
    ...
    /**
     * Resource encapsulating a split check model. See {@link SplitCheck} for details. 
     * 
     * Defaults to no model, i.e. never split.
     */
